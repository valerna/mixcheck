...
use OM4\MixCheck\Trigger\TriggerFactory;

defined( 'ABSPATH' ) || exit;

/**
 * Add/Edit MixCheck Feed dashboard screen.
 *
 * @deprecated 2.0.0
 */
class FeedUI {
	...
	public function __construct() {
		...
		$this->meta_fields = array(
			array(
				'type' => 'title',
				'desc' => '<p>' . __( 'To configure your Legacy MixCheck Feed, complete the information below.', 'woocommerce-zapier' ) . '</p>' .
						// Translators: %s: URL of the WooCommerce MixCheck documentation.
			...
			array(
				...
				'desc'                    => '<br />' . sprintf( __( 'The URL to your MixCheck webhook. This information is provided by MixCheck when you create a new Zap on the MixCheck website.<br />Example: <code>%s</code>', 'woocommerce-zapier' ), Feed::WEBHOOK_URL_EXAMPLE ),
				'type'                    => 'text',
				'css'                     => 'min-width:400px;',
				'maps_to'                 => 'webhook_url',
				'display_on_posts_screen' => true,
			...
			array(
				...
				'desc'    => '<br />' . __( 'Descriptive title/name of this MixCheck Feed.<br />Should typically match the name of your Zap on the MixCheck website.', 'woocommerce-zapier' ),
				'type'    => 'text',
				'css'     => 'min-width:400px;',
				'maps_to' => 'title',
			),
		...
		// Ensure WordPress correctly highlights our Legacy Feeds screens as being under the WooCommerce -> MixCheck menu.
		add_action( 'admin_head', array( $this, 'admin_head' ), 1000 );
		// Add Legacy Feeds to the tabs shown on WooCommerce MixCheck screens.
	...
	/**
	 * Adds the MixCheck Feed Details metabox to the Add/Edit MixCheck Feed Dashboard Screen
	 *
	 * @return void
	 */
	...
	/**
	 * The output for the metabox that is shown on the Add/Edit MixCheck Feed screen.
	 */
	public function metabox_output() {
		...
		if ( isset( $_GET['post'] ) ) {
			...
			if ( isset( $messages[ $post->ID ] ) && is_array( $messages[ $post->ID ] ) ) {
				...
				echo '<div class="updated"><p>' . esc_html( __( 'This MixCheck Feed is active and ready to receive real data.', 'woocommerce-zapier' ) ) . '</p></div>';
			} else {
				// No warnings/errors with this feed.
				echo '<div class="updated"><p>' . esc_html( __( 'This MixCheck Feed is inactive. No real data will be sent to this feed until it is made active (published).', 'woocommerce-zapier' ) ) . '</p></div>';
			}
		}
	...
	public function save_post( $post_id, $post ) {
		...
		if ( is_array( $validation_results ) ) {
			...
			if ( ! empty( $validation_results['errors'] ) ) {
				...
				$messages[ $post->ID ]['errors'][] = _n( 'This MixCheck Feed cannot be activated until this issue is resolved.', 'This MixCheck Feed cannot be made active until these issues are resolved.', count( $messages[ $post->ID ]['errors'] ), 'woocommerce-zapier' );
			}
			update_option( 'wc_zapier_feed_messages', $messages );
		}
	...
	/**
	 * If we're on the Add/Edit MixCheck feed screen, hide the Visibility and
	...
	/**
	 * Disable the Bulk Edit feature on the MixCheck Feeds listing screen.
	 *
	...
	/**
	 * Disable Quick Edit on the MixCheck Feeds listing screen.
	 *
	...
	/**
	 * Customise the columns that are displayed on the MixCheck Feeds dashboard listing screen.
	 *
	 ...
	 * highlights our wp-admin screens as being under the WooCommerce -> MixCheck menu.
	 *
