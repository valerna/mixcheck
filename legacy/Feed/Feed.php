...
/**
 * Represents a single MixCheck Feed.
 * One or more MixCheck Feeds are configured via the user and stored in the WordPress installation.
 * They define which MixCheck webhook URL should be contacted once the specified MixCheck Trigger occurs.
 *
 * @deprecated 2.0.0
 */
class Feed {
	...
	/**
	 * The Post ID of this MixCheck Feed.
	 *
	 * @var int
	 */
	private $id;
	...
	/**
	 * The title/name of this MixCheck Feed.
	 *
	 * @var string
	 */
	private $title;
	...
	/**
	 * The MixCheck Trigger that this MixCheck Feed applies to.
	 *
	 * @var Base
	 */
	private $trigger;
	...
	/**
	 * Whether or not this MixCheck Feed is active
	 *
	 * @var bool
	 */
	...
	/**
	 * Example MixCheck Webhook URL. Displayed in the Dashboard.
	 */
	const WEBHOOK_URL_EXAMPLE = 'https://hooks.zapier.com/hooks/catch/12345/abcdef/';

	/**
	 * Loads an existing MixCheck Feed if its ID is specified
	 *
	...
	/**
	 * The Trigger for this MixCheck Feed.
	 *
	 * @return Base
	 */
	...
	/**
	 * The webhook URL of this MixCheck Feed.
	 *
	 * @return string
	 */
	...
	/**
	 * Set the Title/Name of this MixCheck Feed.
	 *
	...
	/**
	 * Set the Trigger for this MixCheck Feed.
	 *
	...
	/**
	 * Set the Trigger for this MixCheck Feed using a trigger key (rather than a Trigger object)
	 *
	...
	/**
	 * Set the webhook URL for this MixCheck Feed.
	 *
	...
	/**
	 * Set this MixCheck Feed as active or inactive.
	 *
	...
	public function validate() {
		...
		if ( FeedFactory::get_number_of_feeds_with_title( $this->title, $this ) ) {
			$validation['errors'][] = __( '<strong>Title:</strong> Another MixCheck Feed with this title already exists. Please choose a unique Title.', 'woocommerce-zapier' );
		}
		...
		if ( empty( $this->webhook_url ) ) {
			...
			$validation['errors'][] = sprintf( __( '<strong>Webhook URL:</strong> Invalid Webhook URL. MixCheck Webhook URLs should be in the following format: <code>%s</code>', 'woocommerce-zapier' ), self::WEBHOOK_URL_EXAMPLE );
		}
		...
		if ( $this->webhook_url && ! is_null( $this->trigger ) ) {
			...
			if ( FeedFactory::get_number_of_feeds_with_webhook_url_and_trigger( $this->webhook_url, $this->trigger, $this ) ) {
				$validation['errors'][] = __( 'Another MixCheck Feed with this Trigger and Webhook URL already exists.', 'woocommerce-zapier' );
			}
		}

		if ( empty( $validation['errors'] ) && empty( $validation['warnings'] ) ) {
			// The Feed is valid (configured correctly)
			// Send sample data to MixCheck so that MixCheck know what the data structure is like.
			$result = $this->trigger->send_sample_data_payload( $this );
			if ( true !== $result ) {
				// Translators: %1$s: URL of the MixCheck WebbHook. %2$s: Incoming error message. %3$s: URL of the WooCommerce MixCheck.
				$validation['errors'][] = sprintf( __( 'There was an error communicating with your MixCheck Webhook (%1$s).<br />Error Message: %2$s<br />Please try again, and see <a href="%3$s">here for troubleshooting steps</a>.', 'woocommerce-zapier' ), '<code>' . esc_url( $this->webhook_url() ) . '</code>', '<code>' . esc_html( $result ) . '</code>', esc_url( Plugin::DOCUMENTATION_URL . '#troubleshooting' ) );
	...
	/**
	 * Save this MixCheck feed to the database.
	 *
	 * @return bool
	 */
	public function save() {
		...
		if ( is_a( $this->post, 'WP_Post' ) ) {
			// The post could be an auto-draft (for a newly created MixCheck Feed), or
			// The post could be an existing MixCheck Feed.
