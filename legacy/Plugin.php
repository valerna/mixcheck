...
use Exception;
use OM4\MixCheck\Admin\Dashboard;
use OM4\MixCheck\Logger;
use OM4\MixCheck\Plugin\CheckoutFieldEditor;
use OM4\MixCheck\Plugin\Subscriptions;
use OM4\MixCheck\SendQueue;
...
/**
 * Class Plugin
 * This class is a singleton, and should be accessed via the WC_MixCheck() function.
 *
 * @deprecated 2.0.0 Replaced by OM4\WooCommerceMixCheck\Plugin
 */
class Plugin {
	...
	public function plugins_loaded() {
		...
		add_action( 'wc_zapier_resend_sample_data', array( '\\OM4\\MixCheck\\Plugin', 'resend_sample_data' ) );
	}
	...
	/**
	 * Registers the custom post type for storing Legacy MixCheck feeds.
	...
	/**
	 * Prepare (decode) a string, in preparation for sending to MixCheck
	 * Some order fields in WooCommerce are HTML entity encoded, and they need
	 * to be decoded before sending to MixCheck. For example, "&amp;" should be
	 * sent as "&".
	 *
	...
	public static function decode( $value ) {
		if ( is_array( $value ) ) {
			return json_encode( array_map( array( 'OM4\\MixCheck\\Plugin', 'decode' ), $value ) );
		} elseif ( is_object( $value ) ) {
			// TODO: Handle scenarios where an object is passed (Issue #125).
			( new Logger() )->notice( 'OM4\MixCheck\Plugin::decode() value is an object: %s', array( json_encode( $value ) ) );
	...
	/**
	 * Immediately re-send sample data to all currently active MixCheck Feeds that use these triggers
	 * This ensures that MixCheck has the latest checkout field definitions.
	 *
