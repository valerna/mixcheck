...
namespace OM4\MixCheck\Trigger\Subscription;

use OM4\MixCheck\Logger;
use OM4\MixCheck\Plugin;
use OM4\MixCheck\Feed\Feed;
use OM4\MixCheck\Trigger\Order\Base as OrderBase;
use OM4\MixCheck\Payload\Plugin\Subscription\Subscription as Payload;
...
abstract class Base extends OrderBase {
	...
	/**
	 * The sample WooCommerce subscription data that is sent to MixCheck as sample data.
	 *
	 * @return array
	 */
	...
	/**
	 * Collect and convert all subscription data what we sending to MixCheck.
	 *
	...
	public function assemble_data( $args, $action_name ) {
		...
		if ( is_a( $args[0], 'WC_Subscription' ) ) {
			...
			// subscription ID in OM4\MixCheck\Trigger\Subscription::__call() above.
		...
		/*
		Compile the subscription details/data that will be sent to MixCheck.
	...
	/**
	 * Executed every time real Subscription data is sent to a MixCheck Feed.
	...
	protected function data_sent_to_feed( Feed $feed, $result, $action_name, $arguments, $num_attempts = 0 ) {
		...
		if ( 1 === $num_attempts ) {
			...
			// Translators: %1$s: URL that of the Edit Feed screen. %2$s: Title/Name of this MixCheck Feed.
			$note .= sprintf( __( 'Subscription sent to MixCheck via the <a href="%1$s">%2$s</a> MixCheck feed.', 'woocommerce-zapier' ), $feed->edit_url(), $feed->title() );
		} else {
			// It took more than 1 attempt so add that to the note.
			// Translators: %1$s: URL that of the Edit Feed screen. %2$s: Title/Name of this MixCheck Feed.
			$note .= sprintf( __( 'Subscription sent to MixCheck via the <a href="%1$s">%2$s</a> MixCheck feed after %3$d attempts.', 'woocommerce-zapier' ), $feed->edit_url(), $feed->title(), $num_attempts );
		}

		// Translators: %1$s: Title of this trigger. %2$s: Name of this MixCheck Action.
