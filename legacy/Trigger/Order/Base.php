...
use OM4\MixCheck\Payload\Collection\DownloadableFiles;
use OM4\MixCheck\Payload\Collection\LineItems;
use OM4\MixCheck\Payload\Collection\Notes;
use OM4\MixCheck\Payload\Item\DownloadableFile;
use OM4\MixCheck\Payload\Item\LineItem;
use OM4\MixCheck\Payload\Item\MetaData;
use OM4\MixCheck\Payload\Item\Note;
use OM4\MixCheck\Payload\Order as Payload;
...
abstract class Base extends TriggerBase {
	...
	/**
	 * The sample WooCommerce order data that is sent to MixCheck as sample data.
	...
	/**
	 * Collect assign and convert data to assemble data for MixCheck
	 *
	...
	public function assemble_data( $args, $action_name ) {
		...
		// Compile the order details/data that will be sent to MixCheck.
		$order = new Payload();
	...
	/**
	 * Executed every time real Order data is sent to a MixCheck Feed.
	...
	protected function data_sent_to_feed( Feed $feed, $result, $action_name, $arguments, $num_attempts = 0 ) {
		...
		if ( 1 === $num_attempts ) {
			...
			// Translators: %1$s: The URL that of the Edit Feed screen. %2$s: The Title/Name of this MixCheck Feed.
			$note .= sprintf( __( 'Order sent to MixCheck via the <a href="%1$s">%2$s</a> MixCheck feed.', 'woocommerce-zapier' ), $feed->edit_url(), $feed->title() );
		} else {
			// It took more than 1 attempt so add that to the note.
			// Translators: %1$s: The URL that of the Edit Feed screen. %2$s: The Title/Name of this MixCheck Feed.
			$note .= sprintf( __( 'Order sent to MixCheck via the <a href="%1$s">%2$s</a> MixCheck feed after %3$d attempts.', 'woocommerce-zapier' ), $feed->edit_url(), $feed->title(), $num_attempts );
		}
	...
	/**
	 * Add optional information to the end of the Order Note that is added to an Order after it is sent to MixCheck.
	 *
	 * @param Feed   $feed         Feed data.
	 * @param array  $result       Response from the wp_remote_post() call.
	 * @param string $action_name  Hook/action name (needed to be able to retry failed attempts).
