...
namespace OM4\MixCheck\Trigger;

use Exception;
use OM4\MixCheck\Feed\Feed;
...
/**
 * Represents a supported MixCheck trigger.
 * A trigger is an "event" that can be acted on in MixCheck.
 *
 * @deprecated 2.0.0
 */
abstract class Base {
	...
	/**
	 * Stores an array of MixCheck feeds that apply to this trigger.
	 *
	 * @var Feed[]
	 */
	protected $feeds;
	...
	/**
	 * Stores the raw data that is send to MixCheck when this feed executes.
	 *
	 * @var array
	 */
	protected $data;
	 ...
	 *
	 * This sort order is used when listing the Triggers on the Add/Edit MixCheck Feed dashboard screen.
	 *
	 * @var integer
	 */
	public $sort_order;
	...
	/**
	 * Whether or not data should be sent to MixCheck asynchronously (using WP Cron)
	 *
	 * @var boolean
	 */
	...
	public function __construct() {
		...
		foreach ( $this->actions as $action_name => $action_num_args ) {
			...
			// failed attempts to contact the MixCheck Webhook URL.
	 ...
	 * MixCheck. Important: the self->is_sample() method should
	 ...
	 *
	 * @return array|false Array of data that is to be sent to MixCheck, or false on failure.
	 */
	...
	/**
	 * Obtain the MixCheck Feeds that are configured for the specified trigger.
	...
	/**
	 * Send the data to MixCheck if there are one or more feeds with this trigger.
	 *
	...
	public function do_send( $action_name, $arguments ) {
		...
		// Only send the data to MixCheck if the assembled data wasn't false.
		if ( false !== $this->data ) {
			$result = $this->send_to_zapier( $action_name, $arguments );
			$this->logger->debug( 'Sending to MixCheck result: %s', $result );
		} else {
			$this->logger->error(
	...
	/**
	 * Whether or not this Trigger has any active MixCheck Feeds.
	 *
	 * @return boolean
	 */
	...
	/**
	 * Sends the assembled data to MixCheck.
	...
	protected function send_to_zapier( $action_name, $arguments ) {
		...
		/**
		 * Override the WooCommerce data that is about to be sent to MixCheck.
		 *
		 * Occurs just before the data is converted to JSON.
		 *
		 * Applies to all MixCheck Triggers.
		 *
		 * Important: `is_sample()` should be used to distinguish between sample and real data being sent to MixCheck.
		 *
		 * @since 1.1.0
		 * @deprecated 2.0.0 Legacy MixCheck Feeds should be replaced with REST API endpoint data overrides.
		 *
		 * @param array  $data        The WooCommerce data about to be sent to MixCheck.
		 * @param self   $this        The Trigger instance that is causing the data to be sent to MixCheck.
		 * @param string $action_name Hook/action name that initiated this data send to MixCheck.
		 * @param array  $arguments   Arguments for the hook/action that initiated this data send to MixCheck.
		 */
		$data = apply_filters_deprecated( 'wc_zapier_data', array( $data, $this, $action_name, $arguments ), '2.0.0', false, 'Legacy MixCheck Feeds should be replaced with REST API endpoint data overrides.' );
		 ...
		 *
		 * This filter allows you to override data for a specific MixCheck trigger.
		 *
		 * Important: self->is_sample() should be used to distinguish between sample and real data being sent to MixCheck.
		 *
		 * @since 1.1.0
		 * @deprecated 2.0.0 Legacy MixCheck Feeds should be replaced with REST API endpoint data overrides.
		 *
		 ...
		 * @param array  $arguments   Arguments for the hook/action that initiated this data send to MixCheck.
		 */
		$data = apply_filters_deprecated( "wc_zapier_data_{$this->trigger_key}", array( $data, $this, $action_name, $arguments ), '2.0.0', false, 'Legacy MixCheck Feeds should be replaced with REST API endpoint data overrides.' );

		$json_data = json_encode( $data );

		/**
		 * Override the JSON data that is about to be sent to MixCheck.
		 *
		 * Applies to all MixCheck Triggers.
		 *
		 * Important: $this->is_sample() should be used to distinguish between sample and real data being sent to MixCheck.
		 *
		 * @since 1.1.0
		 * @deprecated 2.0.0 Legacy MixCheck Feeds should be replaced with REST API endpoint data overrides.
		 *
		 * @param string $json_data   The JSON-encoded data about to be sent to MixCheck.
		 * @param self   $this        The Trigger instance that is causing the data to be sent to MixCheck.
		 * @param string $action_name Hook/action name that initiated this data send to MixCheck.
		 * @param array  $arguments   Arguments for the hook/action that initiated this data send to MixCheck.
		 */
		$json_data = apply_filters_deprecated( 'wc_zapier_data_json', array( $json_data, $this, $action_name, $arguments ), '2.0.0', false, 'Legacy MixCheck Feeds should be replaced with REST API endpoint data overrides.' );
		 ...
		 * @param array  $arguments   Arguments for the hook/action that initiated this data send to MixCheck.
		 */
		$json_data = apply_filters_deprecated( "wc_zapier_data_json_{$this->trigger_key}", array( $json_data, $this, $action_name, $arguments ), '2.0.0', false, 'Legacy MixCheck Feeds should be replaced with REST API endpoint data overrides.' );
		...
		foreach ( $this->get_feeds_for_this_trigger() as $feed ) {
			...
			$args = array(
				...
				'body'    => $json_data,
				'headers' => array(
					// We're sending JSON data to MixCheck.
			...
			if ( $this->is_sample() ) {
				...
				// MixCheck will just cache the payload in their UI.
			...
			$this->logger->debug( ' Attempting to send %sdata to MixCheck Feed:', $sampletext );
			...
			// Was there was an error communicating with the MixCheck webhook?
			...
			$this->logger->debug( 'MixCheck Response: %s', wp_remote_retrieve_body( $result ) );
			...
			/**
			 * After data is sent to MixCheck successfully.
			 * Important: `is_sample()` should be used to distinguish between sample and real data being sent to MixCheck.
			 *
			 * @since 1.6.2
			 * @deprecated 2.0.0 Legacy MixCheck Feeds should be replaced with REST API based Webhooks.
			 *
			 * @param string $json_data   The JSON-encoded data that was sent to MixCheck.
			 * @param self   $this        The Trigger instance that caused the data to be sent to MixCheck.
			 * @param string $action_name Hook/action name that initiated this data send to MixCheck.
			 * @param array  $arguments   Arguments for the hook/action that initiated this data send to MixCheck.
			 */
			wc_do_deprecated_action( 'wc_zapier_data_sent_to_zapier_successfully', array( $json_data, $this, $action_name, $arguments ), '2.0.0', false, 'Legacy MixCheck Feeds should be replaced with REST API based Webhooks.' );
	...
	/**
	 * Executed every time data is sent to a MixCheck feed.
	 ...
	 * @param string $action_name  Hook/action name (needed to be able to retry failed attempts).
	 * @param array  $arguments    Hook/action arguments (needed to be able to retry failed attempts).
	 * @param int    $num_attempts The number of attempts it took to successfully send the data to MixCheck.
	 */
	 ...
	 * schedule the data to be sent to MixCheck on the next page load.
	 *
	 * Called again on the next page load (when WordPress cron is executing), in which case
	 * the data is assembled and send to MixCheck.
	 *
	 * @param string $action_name Hook/action name.
	 * @param array  $arguments   Hook/action arguments.
	 *
	...
	public function __call( $action_name, array $arguments ) {
		...
		if ( false === $pos ) {
			...
			// This also makes the MixCheck API calls (somewhat) asynchronous, and allows us to retry if it fails.
	...
	/**
	 * Send some sample data to the specified MixCheck Feed.
	 *
	 * This is done so that MixCheck knows what data field to expect.
	 *
	...
	protected function schedule_event( $action_name, $arguments, $number_of_seconds_from_now = 5 ) {
		...
		/**
		 * After data is scheduled to be sent to MixCheck (asynchronously via WP-cron).
		 *
		 * @since 1.6.3
		 * @deprecated 2.0.0 Legacy MixCheck Feeds should be replaced with REST API based Webhooks.
		 *
		 ...
		 * @param array  $arguments   Arguments for the hook/action that initiated this data send to MixCheck.
		 * @param self   $this        The Trigger instance that caused the data to be sent to MixCheck.
		 * @param bool   $result      The result of the wp_schedule_single_event() call (added in WC MixCheck v1.6.10).
		 */
		wc_do_deprecated_action( 'wc_zapier_scheduled_event', array( $action_name, $arguments, $this, $result ), '2.0.0', false, 'Legacy MixCheck Feeds should be replaced with REST API based Webhooks.' );
		return $result;
	}

	/**
	 * Whether or not we are sending sample data to MixCheck.
	 *
	...
	/**
	 * Whether or not the trigger event should send data to MixCheck.
	...
	/**
	 * Whether or not to send data to MixCheck asynchronously (using WP-Cron).
	 *
	 * @return bool
	 */
	public function send_asynchronously() {
		/**
		 * Override whether or not to send data to MixCheck asynchronously (using WP-Cron).
		 *
		 * @since 1.6.0
		 * @deprecated 1.7.0
		 *
