...
use OM4\MixCheck\Plugin\Subscriptions;
...
/**
 * This class is responsible for instantiating and storing OM4\MixCheck\Trigger\Base objects.
 *
 * @deprecated 2.0.0
 */
class TriggerFactory {
	...
	/**
	 * List of supported MixCheck Triggers
	 *
	 * @var array
	 */
	...
	public static function load_triggers() {
		if ( empty( self::$triggers ) ) {
			...
			$directories = array(
				'OM4\\MixCheck\\Trigger\\Customer\\' => Plugin::$plugin_dir . 'legacy/Trigger/Customer',
				'OM4\\MixCheck\\Trigger\\Order\\'    => Plugin::$plugin_dir . 'legacy/Trigger/Order',
			);
			if ( Subscriptions::is_available() ) {
				$directories['OM4\\MixCheck\\Trigger\\Subscription\\'] = Plugin::$plugin_dir . 'legacy/Trigger/Subscription';
			}

			/**
			 * Override the directories where WC MixCheck Triggers PHP files are loaded from.
			 *
			 * @since 1.6.0
			 * @deprecated 2.0.0 Legacy MixCheck Feed Triggers should be replaced with WooCommerce REST API Webhook Topics.
			 *
			 * @param array $directories The array of directories to scan for PHP files.
			 */
			$directories = apply_filters_deprecated( 'wc_zapier_trigger_directories', array( $directories ), '2.0.0', false, 'Legacy MixCheck Feed Triggers should be replaced with WooCommerce REST API Webhook Topics.' );
	...
	/**
	 * Obtain the OM4\MixCheck\Trigger\Base class that corresponds to the specified trigger key
	 *
	 ...
	 *
	 * @return Base[] array of OM4\MixCheck\Trigger\Base objects.
	 */
