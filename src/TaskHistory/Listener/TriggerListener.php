...
namespace OM4\WooCommerceMixCheck\TaskHistory\Listener;
...
use OM4\WooCommerceMixCheck\Plugin\Subscriptions\Note\SubscriptionNote;
...
/**
 * Listener to detect when WooCommerce delivers data to MixCheck via our Webhooks,
 ...
 * if the failed action relates to a MixCheck Webhook send.
 *
 * @since 2.0.0
 */
class TriggerListener {
	...
	/**
	 * Whenever WooCommerce successfully delivers a payload to a WC MixCheck webhook, add the event to our task history.
	 *
	 * Executed by the `woocommerce_webhook_delivery` hook (which occurs for all Webhooks not just MixCheck Webhooks)
	 *
	...
	public function woocommerce_webhook_delivery( $http_args, $response, $duration, $arg, $webhook_id ) {
		...
		if ( is_array( $payload ) && isset( $payload['code'] ) && isset( $payload['message'] ) && isset( $payload['data']['status'] ) ) {
			// The payload (data) sent to MixCheck was a WP_Error.
			// Record the error in the task history as a failed task.
			// MixCheck will also throw an error for this Task, but we want to record the error in our task history too.
			$event = Event::trigger(
				$webhook->get_topic(),
				$topic_name,
		...
		if ( is_wp_error( $response ) ) {
			// Some kind of HTTP error when sending the data to MixCheck.
			$event = Event::trigger(
				$webhook->get_topic(),
				$topic_name,
				// Give the original error message more context.
			...
			/**
			 * The HTTP request was successful, but the response from MixCheck was an error.
			 *
			...
			$event            = Event::trigger(
				...
				new WP_Error(
					...
					sprintf(
						...
						__( 'MixCheck.com returned an unexpected HTTP status code: %1$d (%2$s)', 'woocommerce-zapier' ),
		...
		if ( $is_successful ) {
			 ...
			 * OM4\WooCommerceMixCheck\Webhook\DeliveryFilter::woocommerce_webhook_payload(),
	...
	/**
	 * Whenever an Action Scheduler action execution fails, if the failed action execution relates to a MixCheck webhook delivery,
	 ...
	 * MixCheck webhook delivery, then record the failure in our Task History.
	 *
	 ...
	 * @param  MixCheckWebhook $webhook MixCheck Webhook.
	 *
	 * @return array
	 * @throws InvalidImplementationException If an unknown resource type is encountered.
	 */
	protected function prepare_task_creator_data( $resource_id, $resource_type, MixCheckWebhook $webhook ) {
		$child_id = null;
	 ...
	 * a MixCheck webhook delivery, then record the failure in our Task History.
	 *
	 * @since 2.10.0
	 *
	 * @param  int        $action_id Action Scheduler Action ID.
	...
	protected function action_scheduler_failure( $action_id, $e = null ): void {
		...
		$webhook = new MixCheckWebhook( $args['webhook_id'] );
		if ( ! $webhook->is_zapier_webhook() ) {
			// A non-MixCheck webhook.
			return;
		}
