...
namespace OM4\WooCommerceMixCheck\Plugin\Subscriptions\Note;

use Automattic\WooCommerce\Utilities\OrderUtil;
use OM4\WooCommerceMixCheck\Helper\FeatureChecker;
use OM4\WooCommerceMixCheck\Plugin\Subscriptions\Note\Controller;
...
use OM4\WooCommerceMixCheck\Webhook\Payload;
use OM4\WooCommerceMixCheck\Webhook\Trigger;
use OM4\WooCommerceMixCheck\WooCommerceResource\Base;
...
class SubscriptionNoteResource extends Base {
	...
	/**
	 * Whenever a Woo Subscriptions Note is created, fire the appropriate WooCommerce MixCheck hook if the
	...
	public function subscription_note_added( $subscription_note_id ) {
		...
		/**
		 * Execute the WooCommerce MixCheck handler for the Subscription Note created trigger rule.
		 *
		 * @internal
		 * @since 2.9.0
		 *
	...
	/**
	 * Whenever a WordPress comment is deleted, fire the appropriate WooCommerce MixCheck hook if the
	...
	public function deleted_comment( $comment_id, $comment ) {
		...
		/**
		 * Execute the WooCommerce MixCheck handler for the Subscription Note deleted trigger rule.
		 *
		 * @internal
		 * @since 2.9.0
		 *
	 ...
	 *
	 * @see \OM4\WooCommerceMixCheck\TaskHistory\Listener\TriggerListener::woocommerce_webhook_delivery()
	 *
