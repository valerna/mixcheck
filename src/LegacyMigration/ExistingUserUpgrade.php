...
/**
 * Detects users upgrading from 1.9.x to 2.0+, and if active MixCheck Feeds
 * are found then enable Legacy Mode (which lets the user manage MixCheck Feeds
...
class ExistingUserUpgrade {
	...
	public function initialise() {
		// Enable legacy mode when upgrading from a previous WC MixCheck version.
	...
	/**
	 * If the user has one or more active MixCheck Feeds, then enable Legacy Mode.
	 *
	 * @return void
	 */
	public function enable_legacy_mode_if_required() {
		...
		if ( 0 !== $num ) {
			...
			$this->logger->notice( 'Found %s active MixCheck Feeds. Legacy Mode Enabled.', (string) $num );
	...
	/**
	 * Get the number of active MixCheck Feeds.
	 *
	 * @return int
	 */
	 ...
	 * if it is a MixCheck Feed then check if Legacy Mode can be deactivated.
