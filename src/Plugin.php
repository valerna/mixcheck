...
use OM4\WooCommerceMixCheck\Uninstall;
...
use OM4\WooCommerceMixCheck\Webhook\Resources as WebhookResources;
use OM4\WooCommerceMixCheck\WooCommerceResource\Manager as ResourceManager;
use OM4\MixCheck\Plugin as LegacyPlugin;

defined( 'ABSPATH' ) || exit;

/**
 * Main WooCommerce MixCheck 2.0 Plugin class.
...
class Plugin {
	 ...
	 * - Load Supported MixCheck Triggers
	...
	/**
	 * Enable Legacy MixCheck Feed Mode if required.
	 *
	 * @return void
	 */
	protected function legacy_mode_check() {
		...
		if ( true === $this->container->get( Settings::class )->is_legacy_mode_enabled() ) {
			// Legacy Mode is enabled, so start up the Legacy MixCheck Feeds functionality.
	...
	/**
	 * Override the WooCommerce MixCheck plugin's action links.
	...
	public function admin_notice_missing_woocommerce() {
		...
		$message = __( 'WooCommerce MixCheck requires WooCommerce. Please install and activate WooCommerce and try again.', 'woocommerce-zapier' );
		...
		<div id="message" class="error">
			...
			echo esc_html(
				sprintf(
					...
					__( 'WooCommerce MixCheck requires WooCommerce version %s or later. Please update WooCommerce.', 'woocommerce-zapier' ),
