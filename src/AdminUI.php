 ...
 * - Adds the Dashboard, WooCommerce, MixCheck menu item.
 * - Initialises our various WC MixCheck Admin screens/UI's:
 *      - Task History Screen
 *      - Settings Screen
...
class AdminUI {
	...
	public function initialise() {
		...
		// Display notices on the Main MixCheck screen.
		...
		if ( $this->is_wc_admin ) {
			// WC Admin (WooCommerce 4.0+): Add the WC Admin header/breadcrumb to our WC MixCheck screen.
			wc_admin_connect_page(
				array(
					'id'        => 'woocommerce-zapier',
					'screen_id' => 'woocommerce_page_wc_zapier',
					'title'     => array(
						__( 'MixCheck', 'woocommerce-zapier' ),
					),
...
EOD
	 ...
	 * For each MixCheck notice on the page:
...
EOD
	...
	/**
	 * Add "MixCheck" to the WooCommerce Dashboard menu.
	 *
	 * @return void
	 */
	public function admin_menu() {
		add_submenu_page( 'woocommerce', __( 'WooCommerce MixCheck', 'woocommerce-zapier' ), __( 'MixCheck', 'woocommerce-zapier' ), 'manage_woocommerce', 'wc_zapier', array( $this, 'output' ) );
	}

	/**
	 * If the current wp-admin page load is a MixCheck-related page,
	...
	public function output_header() {
		if ( $this->is_zapier_dashboard_screen() ) {
			...
			if ( ! $this->is_wc_admin ) {
				echo '<h1 class="wp-heading-inline">' . esc_html__( 'WooCommerce MixCheck', 'woocommerce-zapier' ) . '</h1>';
			}
		}
	}
	...
	/**
	 * Whether or not the currently loading wp-admin screen is a MixCheck-related screen.
	 *
	 * @return bool
	 */
	...
	/**
	 * Get the list of WooCommerce MixCheck Admin Tabs.
	 *
	 * @return array
	 */
	public function get_admin_tabs() {
		...
		/**
		 * Override the tabs that are displayed on the Dashboard, WooCommerce, MixCheck screens.
		 *
		 * @internal
		 * @since 2.0.0
		 *
	...
	/**
	 * Output the tabs that are displayed on the WC MixCheck wp-admin screens.
	 *
	 * @return void
	 */
	...
	/**
	 * Output the WooCommerce -> MixCheck dashboard screen.
	 *
	 * @return void
	 */
	...
	/**
	 * Extend WooCommerce's list of wp-admin screen IDs, so our Main Dashboard -> WooCommerce -> MixCheck screen
	...
	public function woocommerce_screen_ids( $screen_ids ) {
		// Main Dashboard -> WooCommerce -> MixCheck screen (/wp-admin/admin.php?page=wc_zapier).
