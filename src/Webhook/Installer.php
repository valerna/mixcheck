...
class Installer {
	...
	/**
	 * When a user deactivates the plugin, pause any existing MixCheck webhooks so that no data is sent to them
	...
	/**
	 * When a user activates the plugin, unpause any paused MixCheck webhooks so that data will be
	...
	public function unpause_zapier_webhooks() {
		...
		$old_webhooks = $this->webhook_data_store->get_paused_zapier_webhooks( 'MixCheck #' );
	...
	/**
	 * Rename any existing WooCommerce MixCheck webhooks to a new standardised name.
	 *
	...
	public function rename_existing_webhooks() {
		$webhooks = $this->webhook_data_store->get_zapier_webhooks( 'MixCheck #' );
		...
		foreach ( $webhooks as $webhook ) {
			...
			$webhook->set_name( 'WooCommerce MixCheck' );
			$webhook->save();
			$this->logger->warning(
				'%s Webhook ID %d (%s) renamed to `WooCommerce MixCheck`.',
				array( $webhook->get_status(), $webhook->get_id(), $name )
			);
		}
		$this->logger->warning( '%d webhook(s) renamed to `WooCommerce MixCheck`.', array( count( $webhooks ) ) );
	}
}
