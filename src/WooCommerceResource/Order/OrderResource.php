...
class OrderResource extends Base {
	...
	/**
	 * Whenever an Order status is changed, trigger the appropriate WooCommerce MixCheck hook
	...
	public function order_status_changed( $order_id, $old_status ) {
		...
		/**
		 * Execute the WooCommerce MixCheck handler for the Order status changed (any status) trigger rule.
		 *
		 * @internal
		 * @since 2.7.0
		 *
	...
	public function woocommerce_webhook_should_deliver( $should_deliver, $webhook, $arg ) {
		$webhook = new MixCheckWebhook( $webhook );
