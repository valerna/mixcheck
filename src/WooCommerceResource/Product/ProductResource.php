...
namespace OM4\WooCommerceMixCheck\WooCommerceResource\Product;
...
class ProductResource extends CustomPostTypeResource {
	...
	/**
	 * Whenever a Product's stock status is changed, trigger the appropriate WooCommerce MixCheck hook
	 * that will then trigger the appropriate MixCheck webhook's Trigger Rule.
	 *
	...
	public function product_stock_status_changed( $product_id, $new_stock_status, WC_Product $product ) {
		...
		/**
		 * Execute the WooCommerce MixCheck handler for the Product stock status changed (any status) trigger rule.
		 *
		 * @internal
		 * @since 2.6.0
		 *
		...
		/**
		 * Execute the WooCommerce MixCheck handler for the Product stock status changed to "X" trigger rule.
		 *
		 * @internal
		 * @since 2.6.0
		 *
	...
	/**
	 * Whenever a Product's stock quantity is changed, trigger the appropriate WooCommerce MixCheck hook
	...
	public function product_stock_quantity_set( WC_Product $product ) {
		...
		if ( $previous_stock_is_not_low && $stock_is_above_no_stock_amount && $new_stock_is_low ) {
			/**
			 * Execute the WooCommerce MixCheck handler for the "Product stock low" trigger rule.
			 *
			 * @internal
			 * @since 2.6.0
			 *
