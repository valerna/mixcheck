...
namespace OM4\WooCommerceMixCheck\Auth;
 ...
 * Auth Key Data Store
 *
 * Responsible for creating, reading and deleting (revoking) MixCheck-specific WooCommerce REST API keys.
 *
 * @since 2.0.0
 */
class KeyDataStore {
	 ...
	 * @var string
	 */
	protected $app_name = 'MixCheck';

	/**
	 * Authentication scope.
	 *
	 * Read/Write access is required so that the MixCheck.com WooCommerce app can
	...
	/**
	 * Database table name where WooCommerce (MixCheck) Authentication Keys are stored.
	 *
	 * @var string
	 */
	...
	/**
	 * Get the existing WooCommerce MixCheck API key(s) for the specified user.
	 *
	...
	public function get_existing_keys( $user_id ) {
		$query = $this->wp_db->prepare(
			...
			$user_id,
			// Description begins with MixCheck.
	...
	/**
	 * Get the number of existing WooCommerce MixCheck API Keys for each user ID.
	 *
	 * @return stdClass[]
	 */
	...
	/**
	 * Get the number of existing WooCommerce MixCheck API Keys.
	 *
	 * @return integer
	 */
